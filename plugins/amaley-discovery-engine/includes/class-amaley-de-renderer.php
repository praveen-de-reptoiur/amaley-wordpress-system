 ...
 *
 * v1.5.4 improves mobile drawer responsiveness and keeps the drawer backdrop outside the transformed form for reliable phone/editor behaviour.
 *
    ...
    class Amaley_DE_Renderer {
        ...
        public function normalize_settings($settings) {
            ...
            $defaults = array(
                ...
                'sort_label' => 'Sort',
                'show_sidebar_cta' => 'no',
                'show_sidebar_cta_kicker' => 'yes',
                'show_sidebar_cta_title' => 'yes',
                'show_sidebar_cta_text' => 'yes',
                'show_sidebar_cta_button' => 'yes',
        ...
        private function device_filter_css($id, $mode, $device, $position = 'top') {
            ...
            if ('drawer' === $mode) {
                ...
                $css .= $base . ' .amaley-discovery-engine-v1__filters{display:flex!important;position:fixed!important;top:var(--ade-drawer-edge,14px)!important;right:var(--ade-drawer-edge,14px)!important;bottom:var(--ade-drawer-edge,14px)!important;left:auto!important;transform:translate3d(calc(100% + 32px),0,0)!important;width:min(var(--ade-drawer-width,430px),calc(100vw - var(--ade-drawer-edge,14px) - var(--ade-drawer-edge,14px)))!important;max-width:calc(100vw - var(--ade-drawer-edge,14px) - var(--ade-drawer-edge,14px))!important;height:auto!important;max-height:calc(100dvh - var(--ade-drawer-edge,14px) - var(--ade-drawer-edge,14px))!important;overflow:auto!important;z-index:99999!important;border-radius:var(--ade-drawer-radius,24px)!important;background:var(--ade-drawer-bg,var(--ade-filter-bg,#FFF8EA))!important;box-shadow:var(--ade-drawer-shadow,0 24px 70px rgba(46,18,3,.26))!important;-webkit-overflow-scrolling:touch!important;}';
        ...
        private function render_filters($settings) {
            ...
            <form class="amaley-discovery-engine-v1__filters" data-ade-form method="get">
                ...
                <?php if ('yes' === $settings['show_search']) : ?><label class="amaley-discovery-engine-v1__field amaley-de-field-search"><span><?php echo esc_html($settings['search_label']); ?></span><input type="search" name="ade_search" value="<?php echo esc_attr($filters['search']); ?>" placeholder="<?php echo esc_attr($settings['search_placeholder']); ?>"></label><?php endif; ?>
        ...
        private function render_sidebar_cta($settings) {
            ...
            if ('yes' !== ($settings['show_sidebar_cta_desktop'] ?? 'yes')) { $classes[] = 'hide-desktop'; }
            if ('yes' !== ($settings['show_sidebar_cta_tablet'] ?? 'no')) { $classes[] = 'hide-tablet'; }
            if ('yes' !== ($settings['show_sidebar_cta_mobile'] ?? 'no')) { $classes[] = 'hide-mobile'; }
            $cta_url = !empty($settings['sidebar_cta_button_url']) ? $settings['sidebar_cta_button_url'] : '#';
            ob_start(); ?>
            <div class="<?php echo esc_attr(implode(' ', $classes)); ?>">
                <?php if ('yes' === ($settings['show_sidebar_cta_kicker'] ?? 'yes') && !empty($settings['sidebar_cta_kicker'])) : ?><small class="amaley-discovery-engine-v1__sidebar-cta-kicker"><?php echo esc_html($settings['sidebar_cta_kicker']); ?></small><?php endif; ?>
                <?php if ('yes' === ($settings['show_sidebar_cta_title'] ?? 'yes') && !empty($settings['sidebar_cta_title'])) : ?><strong class="amaley-discovery-engine-v1__sidebar-cta-title"><?php echo esc_html($settings['sidebar_cta_title']); ?></strong><?php endif; ?>
                <?php if ('yes' === ($settings['show_sidebar_cta_text'] ?? 'yes') && !empty($settings['sidebar_cta_text'])) : ?><p class="amaley-discovery-engine-v1__sidebar-cta-text"><?php echo wp_kses_post($settings['sidebar_cta_text']); ?></p><?php endif; ?>
                <?php if ('yes' === ($settings['show_sidebar_cta_button'] ?? 'yes') && !empty($settings['sidebar_cta_button_text'])) : ?><a class="amaley-discovery-engine-v1__sidebar-cta-button" href="<?php echo esc_url($cta_url); ?>"><?php echo esc_html($settings['sidebar_cta_button_text']); ?></a><?php endif; ?>
            </div>
            <?php return ob_get_clean();
        }
        ...
        private function render_term_select($name, $taxonomy, $label, $current, $settings) {
            ...
            if (is_wp_error($terms)) { return ''; }
            ob_start(); ?>
            <label class="amaley-discovery-engine-v1__field <?php echo esc_attr($this->field_class_from_name($name)); ?>"><span><?php echo esc_html($label); ?></span><select name="<?php echo esc_attr($name); ?>"><option value=""><?php echo esc_html($settings['all_option_text']); ?></option><?php foreach ($terms as $term) : ?><option value="<?php echo esc_attr($term->slug); ?>" <?php selected($current, $term->slug); ?>><?php echo esc_html($term->name); ?></option><?php endforeach; ?></select></label>
            <?php return ob_get_clean();
        }
        ...
        private function field_class_from_name($name) {
            $key = str_replace(array('_', 'ade-'), array('-', ''), sanitize_key($name));
            return 'amaley-de-field-' . sanitize_html_class(str_replace('_', '-', (string) $name));
        }
