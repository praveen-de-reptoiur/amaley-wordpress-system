    ...
    class Amaley_DE_Query {
        ...
        private function query_products($settings, $filters) {
            ...
            foreach ($this->attribute_taxonomy_map() as $attr_key_for_scope => $attr_taxonomy_for_scope) {
                $this->apply_tax_include_exclude($args, $attr_taxonomy_for_scope, $settings['include_attr_' . $attr_key_for_scope] ?? '', $settings['exclude_attr_' . $attr_key_for_scope] ?? '');
            }
            ...
            if ($tag) {
                $args['tax_query'][] = array('taxonomy' => 'product_tag', 'field' => 'slug', 'terms' => $tag);
            }
            ...
            if (!empty($filters['attrs']) && is_array($filters['attrs'])) {
                foreach ($this->attribute_taxonomy_map() as $attr_key => $taxonomy) {
                    ...
                    $args['tax_query'][] = array('taxonomy' => $taxonomy, 'field' => 'slug', 'terms' => sanitize_title($filters['attrs'][$attr_key]));
                }
            }

            $stock = sanitize_key($filters['stock'] ?? '');
            if ($stock) {
                $args['meta_query'][] = array('key' => '_stock_status', 'value' => $stock);
            }
            ...
            if (null !== $min_price || null !== $max_price) {
                $args['meta_query'][] = array('key' => '_price', 'type' => 'NUMERIC', 'compare' => 'BETWEEN', 'value' => array(null !== $min_price ? $min_price : 0, null !== $max_price ? $max_price : 999999999));
            }
            ...
            return array('items' => $query->posts, 'total' => (int) $query->found_posts, 'pages' => (int) $query->max_num_pages, 'current' => $page);
        }
        ...
        private function query_posts($settings, $filters) {
            ...
            $per_page = max(1, min(96, absint($settings['per_page'] ?? 9)));
            $sort = sanitize_key($filters['sort'] ?? ($settings['default_sort'] ?? 'latest'));
            $args = array('post_type' => $post_type, 'post_status' => 'publish', 'posts_per_page' => $per_page, 'paged' => $page, 's' => sanitize_text_field($filters['search'] ?? ''), 'tax_query' => array(), 'meta_query' => array());
            $category = sanitize_title($filters['category'] ?? '');
            if ($category && $taxonomy && taxonomy_exists($taxonomy)) {
                $args['tax_query'][] = array('taxonomy' => $taxonomy, 'field' => 'slug', 'terms' => $category);
            }
            if (!empty($settings['relation_meta_key']) && !empty($settings['relation_meta_value'])) {
                $args['meta_query'][] = array('key' => sanitize_key($settings['relation_meta_key']), 'value' => sanitize_text_field($settings['relation_meta_value']));
            }
            if (count($args['tax_query']) > 1) { $args['tax_query']['relation'] = 'AND'; }
            if (count($args['meta_query']) > 1) { $args['meta_query']['relation'] = 'AND'; }
            if ('title' === $sort) { $args['orderby'] = 'title'; $args['order'] = 'ASC'; } else { $args['orderby'] = 'date'; $args['order'] = 'DESC'; }
            $query = new WP_Query($args);
            return array('items' => $query->posts, 'total' => (int) $query->found_posts, 'pages' => (int) $query->max_num_pages, 'current' => $page);
        }
        ...
        private function apply_tax_include_exclude(&$args, $taxonomy, $include_slugs = '', $exclude_slugs = '') {
            ...
            if (!empty($include)) { $args['tax_query'][] = array('taxonomy' => $taxonomy, 'field' => 'slug', 'terms' => $include, 'operator' => 'IN'); }
            if (!empty($exclude)) { $args['tax_query'][] = array('taxonomy' => $taxonomy, 'field' => 'slug', 'terms' => $exclude, 'operator' => 'NOT IN'); }
        }

        public function attribute_taxonomy_map() {
            return array(
                'cluster' => 'pa_cluster',
        ...
        private function default_post_type_for_type($type) {
            $map = array('collections' => Amaley_DE_Settings::get_value('collection_post_type', 'amaley_collection'), 'clusters' => Amaley_DE_Settings::get_value('cluster_post_type', 'amaley_cluster'), 'shgs' => Amaley_DE_Settings::get_value('shg_post_type', 'amaley_shg'), 'members' => Amaley_DE_Settings::get_value('member_post_type', 'amaley_member'));
            return isset($map[$type]) ? $map[$type] : 'post';
        }

        private function default_taxonomy_for_type($type) {
            $map = array('collections' => Amaley_DE_Settings::get_value('collection_taxonomy', 'amaley_collection_type'), 'clusters' => Amaley_DE_Settings::get_value('cluster_taxonomy', 'amaley_region'), 'shgs' => Amaley_DE_Settings::get_value('shg_taxonomy', 'amaley_shg_category'), 'members' => Amaley_DE_Settings::get_value('member_taxonomy', 'amaley_member_skill'));
