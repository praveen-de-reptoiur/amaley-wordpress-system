<?php
/**
 * Amaley Core — Member / Producer Single Linked SHG Elementor widget.
 *
 * v1.0.122-linked-shg-clean-controls
 * - Live-site safe complete file replacement.
 * - Scope: Single Member / Producer Detail page → Linked SHG section only.
 * - Removes unrelated generic controls from this widget.
 * - Keeps only controls for actual Linked SHG elements: section shell, heading, SHG card,
 *   image/badge, card text, meta boxes, card button, OG Card 1 and fallback message.
 * - No post data, SHG mapping, products, gallery, archive page, WooCommerce or URL logic changes.
 */
...
class Amaley_Core_Member_Single_SHG_Widget extends \Elementor\Widget_Base {
    ...
    public function get_keywords() { return array( 'amaley', 'member', 'producer', 'single', 'shg', 'collective' ); }
    ...
    private function register_source_controls( $defaults ) {
        $this->start_controls_section( 'source_section', array(
            'label' => esc_html__( 'Linked SHG Data / Preview Source', 'amaley-core' ),
        ) );
    ...
    private function register_content_controls( $defaults ) {
        $this->start_controls_section( 'content_section', array(
            'label' => esc_html__( 'Linked SHG Content / Display', 'amaley-core' ),
        ) );

        $this->add_switch_if( $defaults, 'show_section', 'Show Linked SHG Section' );
        ...
        $this->add_control( 'card_display_heading', array(
            'label'     => esc_html__( 'Linked SHG Card Display', 'amaley-core' ),
            'type'      => \Elementor\Controls_Manager::HEADING,
            'separator' => 'before',
        ) );

        $this->add_control( 'card_template', array(
            'label'       => esc_html__( 'SHG Card Template', 'amaley-core' ),
            'type'        => \Elementor\Controls_Manager::SELECT,
            'default'     => isset( $defaults['card_template'] ) ? $defaults['card_template'] : 'current_existing',
            'options'     => array(
                'current_existing' => esc_html__( 'Current / Existing SHG Card', 'amaley-core' ),
        ...
        $this->add_switch_if( $defaults, 'show_card_badge', 'Show Verified Badge', array( 'card_template' => 'current_existing' ) );
    ...
    private function register_section_style_controls() {
        $s = '{{WRAPPER}} .amms-related-shg';

        $this->start_controls_section( 'style_section_shell', array(
            'label' => esc_html__( 'Linked SHG Section / Background', 'amaley-core' ),
        ...
        $this->start_controls_section( 'style_layout', array(
            'label' => esc_html__( 'Linked SHG Layout / Spacing', 'amaley-core' ),
            'tab'   => \Elementor\Controls_Manager::TAB_STYLE,
        ) );
    ...
    private function register_heading_style_controls() {
        ...
        $this->start_controls_section( 'style_heading_block', array(
            'label' => esc_html__( 'Linked SHG Heading / Text', 'amaley-core' ),
            'tab'   => \Elementor\Controls_Manager::TAB_STYLE,
        ) );
    ...
    private function register_current_card_style_controls() {
        $s = '{{WRAPPER}} .amms-related-shg';
        $card = $s . ' .amms-related-card.amms-shg-card';

        $this->start_controls_section( 'style_current_card_box', array(
            'label'     => esc_html__( 'Current SHG Card / Box', 'amaley-core' ),
            'tab'       => \Elementor\Controls_Manager::TAB_STYLE,
        ...
        $this->start_controls_section( 'style_current_card_media', array(
            'label'     => esc_html__( 'Current SHG Card Image / Badge', 'amaley-core' ),
            'tab'       => \Elementor\Controls_Manager::TAB_STYLE,
            'condition' => array( 'card_template' => 'current_existing' ),
        ) );
        ...
        $this->add_control( 'related_badge_color', array( 'label' => esc_html__( 'Badge Text Color', 'amaley-core' ), 'type' => \Elementor\Controls_Manager::COLOR, 'selectors' => array( $s . ' .amms-related-media b' => 'color: {{VALUE}} !important;' ) ) );
        $this->add_control( 'related_badge_bg', array( 'label' => esc_html__( 'Badge Background', 'amaley-core' ), 'type' => \Elementor\Controls_Manager::COLOR, 'selectors' => array( $s . ' .amms-related-media b' => 'background: {{VALUE}} !important;' ) ) );
        $this->end_controls_section();

        $this->start_controls_section( 'style_current_card_text', array(
            'label'     => esc_html__( 'Current SHG Card Text', 'amaley-core' ),
            'tab'       => \Elementor\Controls_Manager::TAB_STYLE,
            'condition' => array( 'card_template' => 'current_existing' ),
        ) );
        ...
        $this->start_controls_section( 'style_current_card_meta', array(
            'label'     => esc_html__( 'Current SHG Detail Boxes', 'amaley-core' ),
            'tab'       => \Elementor\Controls_Manager::TAB_STYLE,
            'condition' => array( 'card_template' => 'current_existing' ),
        ) );
        ...
        $this->start_controls_section( 'style_current_card_button', array(
            'label'     => esc_html__( 'Current SHG Card Button', 'amaley-core' ),
            'tab'       => \Elementor\Controls_Manager::TAB_STYLE,
            'condition' => array( 'card_template' => 'current_existing' ),
        ) );
    ...
    private function register_og_card_style_controls() {
        ...
        $this->start_controls_section( 'style_og_card_fine_controls', array(
            'label'     => esc_html__( 'OG SHG Card 1 Controls', 'amaley-core' ),
            'tab'       => \Elementor\Controls_Manager::TAB_STYLE,
        ...
        $this->add_control( 'og_card_note', array(
            ...
            'raw'             => esc_html__( 'These controls appear only when SHG Card Template is set to OG SHG Card 1.', 'amaley-core' ),
    ...
    private function register_fallback_style_controls() {
        ...
        $this->start_controls_section( 'style_fallback', array(
            'label' => esc_html__( 'Linked SHG Fallback / Empty Message', 'amaley-core' ),
            'tab'   => \Elementor\Controls_Manager::TAB_STYLE,
        ) );
